...
# Updated import to match the finalized model
from models.clubs import ClubResponse  # Corrected Pydantic model
...
async def get_club_firestore_document(  # This function fetches a single club's data
    db: Client, club_id: str
) -> Optional[Dict[str, Any]]:
    """
    Fetches a single club document from Firestore by club ID.
    ...
    Other keys should match ClubResponse attributes (e.g., logoURL, contactEmail).
    """
    if not club_id:
        return None
    club_doc_ref = db.collection("clubs").document(club_id)
    club_doc = await asyncio.to_thread(club_doc_ref.get)
    if club_doc.exists:
...
async def get_user_joined_club_details(
    ...
    """
    Retrieves the full details for all clubs a user has joined.
    """
    ...
    for club_data_from_firestore in fetched_club_data_list:
        if club_data_from_firestore:
            try:
                # Firestore data keys (name, description, category, contactEmail,
                # logoURL, memberCount) already match the ClubResponse attributes.
                # club_data_from_firestore also includes "clubId" from get_club_firestore_document.
